...
# Persistent SECRET_KEY helper to prevent worker session drops
...
# Use absolute path for local SQLite database to prevent CWD mismatches on
# WSGI servers
...
if not db_url:
    ...
    db_url = 'sqlite:///' + os.path.join(instance_dir, 'events.db')
...
class Student(db.Model):
    ...
    waitlist_entries = db.relationship(
        'Waitlist', backref='student', lazy=True)
    ...
    if filter_date:
        try:
            ...
            query = query.filter(db.func.date(Event.date)
                                 == filter_datetime.date())
        except ValueError:
            pass

    events = query.filter(
        Event.date >= datetime.utcnow()).order_by(
        Event.date.asc()).all()
...
def register(event_id):
    ...
    if request.method == 'POST':
        ...
        existing_reg = Registration.query.filter_by(
            event_id=event_id, student_id=student.id).first()
        ...
        if event.is_full():
            ...
            flash(
                'Registration successful! Check your email for confirmation and QR code.',
                'success')
...
def generate_qr(token):
    registration = Registration.query.filter_by(
        qr_code_token=token).first_or_404()
...
def edit_event(event_id):
    ...
    if request.method == 'POST':
        ...
        event.date = datetime.strptime(
            request.form.get('date'), '%Y-%m-%dT%H:%M')
...
def cancel_registration(reg_id):
    ...
    if waitlist_entry:
        ...
        remaining_waitlist = Waitlist.query.filter_by(
            event_id=event.id).order_by(
            Waitlist.position).all()
...
def event_library():
    ...
    events_with_folders = [
        event for event in events_with_folders if event.folders]
...
@app.route('/admin/event/<int:event_id>/create-folder',
           methods=['GET', 'POST'])
@login_required
...
@app.route('/admin/event/<int:event_id>/folder/<int:folder_id>/upload-images',
           methods=['GET', 'POST'])
@login_required
    ...
    if request.method == 'POST':
        ...
        for file in files:
            if file and allowed_file(file.filename):
                ...
                filename = f"{folder_id}_{
                    int(
                        datetime.utcnow().timestamp())}_{filename}"
...
# Initialize database and default admin user on import
init_db()
