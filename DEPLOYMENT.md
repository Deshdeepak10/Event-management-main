...
### Option 3: Deploying on PythonAnywhere (For username: Deshdeepak)

PythonAnywhere is a fully managed cloud hosting platform specifically for Python web applications.

#### 1. Upload Your Files
You can clone the code directly from GitHub or upload it as a ZIP file.
To clone using git, open a **Bash Console** on PythonAnywhere and run:
```bash
git clone https://github.com/<your-username>/<your-repo-name>.git ~/Event-management-main
```
*(If you upload a ZIP, make sure it is extracted to `~/Event-management-main`)*
...
mkvirtualenv --python=python3.10 event_env

# Install requirements (excluding platform-incompatible dependencies like gunicorn)
pip install Flask Flask-SQLAlchemy Flask-Login Werkzeug qrcode Pillow python-dotenv pymysql
```

#### 3. Setup Web Application on PythonAnywhere
Go to the **Web** tab in your PythonAnywhere dashboard:
1. Click **Add a new web app**.
2. Select **Manual Configuration** (do *not* choose Flask - manual configuration allows virtual environments).
3. Choose the Python version matching your virtual environment (e.g., `Python 3.10`).
4. Once created, configure the following fields under the Web tab:
   * **Source code**: `/home/Deshdeepak/Event-management-main/Event-management-main`
   * **Working directory**: `/home/Deshdeepak/Event-management-main/Event-management-main`
   * **Virtualenv**: `/home/Deshdeepak/.virtualenvs/event_env`

#### 4. Configure the WSGI file
In the **Web** tab, locate the **WSGI configuration file** link (usually `/var/www/Deshdeepak_pythonanywhere_com_wsgi.py`). Click it, delete all its content, and replace it with:
```python
import sys
import os
...
# Optional: Set environment variables (uncomment and replace if needed)
...
To ensure that stylesheets and uploaded files serve correctly, add the following mappings under the **Static files** section of the **Web** tab:
1. **URL**: `/static/`
   * **Directory**: `/home/Deshdeepak/Event-management-main/Event-management-main/static`

#### 6. Reload and Verify
Click the green **Reload** button at the top of the Web tab. Your application will be live at `http://Deshdeepak.pythonanywhere.com`!
